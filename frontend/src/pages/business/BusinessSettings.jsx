...
import {
  Home, Package, ShoppingCart, TrendingUp, MessageCircle, BarChart3, User as UserIcon,Star
} from 'lucide-react';
...
const BusinessSettings = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      // Don't leave settings null — use empty object so toggles won't crash
      setSettings({});
    } finally {
      setLoading(false);
    }
        ...
        <div className="lg:col-span-3">
          ...
          {activeTab === 'notifications' && (
            <Card>
              ...
              <div className="space-y-4">
                {[
                  { key: 'email_notifications', title: 'Email Notifications',  sub: 'Receive email updates about orders' },
                  { key: 'order_notifications', title: 'Order Updates',         sub: 'Get notified when you receive new orders' },
                  { key: 'low_stock_alerts',    title: 'Low Stock Alerts',      sub: 'Alert when products are running low' },
                ].map(({ key, title, sub }) => (
          ...
          {activeTab === 'privacy' && (
            <Card>
              ...
              <div className="space-y-4">
                {[
                  { key: 'show_contact', title: 'Show Contact Information', sub: 'Display your phone and email to customers' },
                  { key: 'show_address', title: 'Show Business Address',    sub: 'Make your location visible on profile' },
                ].map(({ key, title, sub }) => (
                  <label key={key} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg cursor-pointer">
                    <div>
                      <p className="font-medium text-gray-900">{title}</p>
                ...
                <Button variant="primary" onClick={handleSaveSettings}>
                  <Save className="w-5 h-5 mr-2" />Save Privacy Settings
                </Button>
              </div>
            </Card>
          )}
